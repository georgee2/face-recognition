...
import 'dart:io';
import 'package:final_bassem/Screens/splash_screen.dart';
import 'package:final_bassem/handle_attendance.dart';
...
class MyHomePage extends StatefulWidget {
  ...
  const MyHomePage({Key? key, required this.inOut}) : super(key: key);
    ...
    return Scaffold(
      ...
      floatingActionButton:
        ...
        FloatingActionButton(
          onPressed: () {
            if(_faceFound) {
              submitPerson(_scanResults.keys.toString().substring(1, _scanResults.keys.toString().length -1));
            }
          },
          heroTag: null,
    ...
    var alert = AlertDialog(
      ...
      actions: <Widget>[
        TextButton(
            ...
            onPressed: () {
              if(_scanResults.keys.toString() != '(No Face saved)' && _scanResults != '(NOT RECOGNIZED)') {
                widget.inOut == 'in'?
                HandleAttendance().submitSignIn(_scanResults.keys.toString().substring(1, _scanResults.keys.toString().length -1),
                    '${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}', 'At: ${DateTime.now().hour}:${DateTime.now().minute}')
                : HandleAttendance().submitSignOut(_scanResults.keys.toString().substring(1, _scanResults.keys.toString().length -1),
                    '${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}', 'At: ${DateTime.now().hour}:${DateTime.now().minute}');
              }else{
